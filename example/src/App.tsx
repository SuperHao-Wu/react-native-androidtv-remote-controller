...
import {
	...
	RemoteKeyCode,
	RemoteDirection
} from "react-native-androidtv-remote";
import {
	GoogleCastDiscovery,
	DeviceInfo
} from "./services/GoogleCastDiscovery";
...
function App(): React.JSX.Element {
	...
	const certificateRef = useRef<
		Map<string, { key: string | null; cert: string | null }>
	>(new Map());
	...
	const handleConnect = () => {
		...
		const options = {
			pairing_port: 6467,
			remote_port: 6466,
			service_name: "com.hao.androidtv",
			systeminfo: {
	...
	const handlePairingCodeSubmit = async (pairingCode: string | null) => {
		...
		if (!selectedDevice || !androidRemotesRef.current.has(selectedDevice))
			return;
	...
	return (
		<SafeAreaView style={styles.container}>
			<Text style={styles.statusText}>
				Status: {connectionStatuses[selectedDevice] || "Disconnected"}
			</Text>
